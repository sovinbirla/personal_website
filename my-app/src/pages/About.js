import React, { useState } from "react";
import App from "../App";
import "../styles/About.css";
import { Link } from "react-router-dom";

function About() {
  const [toggleState, setToggleState] = useState(1);

  const toggleTab = (index) => {
    console.log(index);
  };

  return (
    <div class="main" id="about">
      <div class="main__container">
        <div class="main__img--container">
          <div class="main__img--card">
            {/* <i class="fas fa-layer-group"> */}
            <img src="data:image/jpeg;base64," />{" "}
          </div>
        </div>
        <div class="about__content">
          <h1>Who am I?</h1>
          {/* <h2>I am a software engineer.</h2> */}
          <div class="UIUC">
            <p>
              I did my undergrad at Univeristy of Illinois Urbana Champaign
              where I majored in Statistics and Computer Science. I took some
              really cool courses and built some projects.
            </p>

            {/* <div className="uiuc-tabs">
              <div
                className={toggleState === 1 ? "uiuc active-tabs" : "uiuc"}
                onClick={() => toggleTab(1)}>
                Coursework
              </div>
              <div
                className={toggleState === 2 ? "uiuc active-tabs" : "uiuc"}
                onClick={() => toggleTab(2)}>
                Cool Projects
              </div>
            </div>

            <div className="uiuc_content_tabs">
              <div
                className={
                  toggleState === 1
                    ? "uiuc_coursework_tabs active-tabs"
                    : "uiuc_coursework_tabs"
                }>
                <ul>
                  <li>Data Structures(CS 225)</li>
                  <li>Algorithms(CS 374)</li>
                  <li>Applied Machine Learning(CS 498)</li>
                  <li>Time Series Analysis(STAT 429)</li>
                  <li>Numerical Methods(CS 357)</li>
                </ul>
              </div>

              <div
                className={
                  toggleState === 1
                    ? "uiuc_projects_tabs active-tabs"
                    : "uiuc_projects_tabs"
                }>
                <ul>
                  <li>
                    Monte Carlo Simulation Poker: I built the entire gameplay of
                    Poker and used Markov Chain Monte Simulation to calculate
                    the winning hand probabilities of each player after each
                    round of play.
                  </li>
                  <li>
                    Github Social Media: Built a react app and used axios to
                    build a social media app for software developers on campus
                    by templating Instagram's user profile.
                  </li>
                  <li>
                    UNO Game: Built an interactive visualization for UNO
                    gameplay in Android Studio.
                  </li>
                  <li>
                    Built a GraphQL database of the top x number of books and
                    authors on Goodreads.
                  </li>
                </ul>
              </div>
            </div> */}

            <button class="main__btn">
              <Link to="/contact">Contact Me</Link>
            </button>
          </div>
          <div class="Gtech">
            <p>
              I am an incoming student at the Georgia Institute of Technology
              majoring in Quantitative and Computational Finance. Prior to
              graduate school, I worked as a software engineering consultant for
              2 years. These 2 years gave me experience building cloud solutions
              for clients across the world. I like coding, playing tennis, and
              going out on town and exploring city landscapes.
            </p>
          </div>
        </div>
      </div>
    </div>
  );
}

export default About;
